...
glove_model = KeyedVectors.load('data/glove-twitter-100.model')

# load the .npy file array 
glove_model.vectors = np.load('data/glove-twitter-100.model.vectors.npy')
...
def tokens_to_vectors(tokens, model):
    ...
    for token in tokens:
        ...
        if token in model.key_to_index:
            ...
            # Handle out-of-vocabulary tokens by ignoring them
            pass
    return vectors
...
# load the .npy file explicitly, you can use the following:
glove_model.vectors = np.load('data/glove-twitter-100.model.vectors.npy')
...
# Create a bar chart
plt.figure(figsize=(10, 6))
plt.bar(topic_labels, topic_proportions, color='skyblue')
plt.xlabel('Topics')
...
p1 = figure(width=1000, height=800, x_range=unique_topics, tools="")
# Adjust the title
...
# Create a bar chart
plt.figure(figsize=(10, 6))
plt.bar(topic_labels, topic_proportions, color='salmon')
plt.xlabel('Topics')
plt.ylabel('Proportion of Tweets')
plt.title('Proportion of Tweets by Topic')
plt.xticks(rotation=45)
...
p2 = figure(width=1000, height=800, x_range=unique_topics, tools="")
# Adjust the title
